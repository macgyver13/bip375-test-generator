...
`bip375_test_vectors.json` has three vector arrays:

- `valid` / `invalid` — one-shot config-driven PSBTs (see YAML config below).
- `workflows` — a single PSBT walked **step by step** through the BIP-375 roles
  (created → constructed → updated → sp_finalized → signed → finalized →
  transaction). Generated by `WorkflowVectorGenerator` using `spdk_psbt`'s
  high-level role methods.

Each workflow entry is an **atomic step vector**:

```jsonc
{
  "step": "updated",
  "description": "Updater adds UTXO/pubkey data plus ECDH shares and DLEQ proofs",
  "supplementary": { "inputs": [...], "sp_proofs": [...], "outputs": [...] },
  "expected": { "psbt": "<base64>" }   // plus "tx": "<hex>" on the transaction step
}
```

`supplementary` uses the same field shapes as the `valid`/`invalid` vectors but is
**scoped to the material that becomes meaningful at that step** (e.g. `sp_proofs`
first appear at `updated`, output `script` at `sp_finalized`). It is diagnostic
only. `expected.psbt` is the base64 PSBT after that role runs.
